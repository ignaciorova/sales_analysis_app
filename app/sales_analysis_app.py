...
if df.empty:
    st.warning("No se encontraron datos. Asegúrese de que el archivo Excel esté disponible en app/data/.")
else:
    ...
    product_types = ['Todos'] + sorted(
        df['Líneas de la orden'].dropna().astype(str).unique().tolist()
    )
    selected_product = st.sidebar.selectbox("Tipo de Producto", product_types)
    
    client_groups = ['Todos'] + sorted(
        df['Cliente/Nombre principal'].dropna().astype(str).unique().tolist()
    )
    selected_client_grp = st.sidebar.selectbox("Grupo de Clientes", client_groups)
    
    days_of_week = ['Todos'] + sorted(
        df['Día de la Semana'].dropna().astype(str).unique().tolist()
    )
    selected_day = st.sidebar.selectbox("Día de la Semana", days_of_week)
    
    clients = ['Todos'] + sorted(
        df['Cliente/Nombre'].dropna().astype(str).unique().tolist()
    )
    ...
    lunch_df = filtered_df[
        filtered_df['Líneas de la orden'] == 'Almuerzo Ejecutivo Aseavna'
    ].copy()
    lunch_df['Fecha_Dia'] = lunch_df['Fecha'].dt.date
    dup = lunch_df.groupby(
        ['Cliente/Nombre','Fecha_Dia']
    ).filter(lambda x: len(x) > 1)
    
    if not dup.empty:
        st.markdown(
            '<div class="alert-box">⚠️ Se detectaron almuerzos ejecutivos duplicados:</div>',
            unsafe_allow_html=True
        )
        summary = dup.groupby(
            ['Cliente/Nombre','Fecha_Dia']
    ...
    unusual = client_sales[
        client_sales['Ventas Totales (₡)'] > avg_client_sales * 2
    ]
    if not unusual.empty:
        st.markdown(
            '<div class="alert-box">⚠️ Clientes con volumen de compras inusual:</div>',
            unsafe_allow_html=True
        )
    ...
    with c3:
        buf_pdf2 = generate_pdf(
            client_sales,
            "Reporte de Ventas por Cliente - ASEAVNA",
    ...
    )['Monto Neto'].sum().reset_index(name='Monto Neto')
    daily['Days'] = (
        pd.to_datetime(daily['Fecha']) - pd.to_datetime(daily['Fecha'].min())
    ).dt.days
    ...
    if len(daily) > 1:
        ...
        future_dates = [
            pd.to_datetime(daily['Fecha']).max() + timedelta(days=i)
            for i in range(1, 8)
        ]
        ...
        trends = filtered_df.groupby(
            ['Líneas de la orden', filtered_df['Fecha'].dt.to_period('M')]
        )['Monto Neto'].sum().unstack(fill_value=0)
        
        # Verificar si hay al menos dos columnas (dos meses)
        if trends.shape[1] >= 2:
            growth = (
                (trends.iloc[:,-1] - trends.iloc[:,-2]) /
                trends.iloc[:,-2].replace(0, np.nan) * 100
            ).replace([np.inf, -np.inf], 0).dropna().sort_values(ascending=False)
    ...
    # Top 10 productos
    top10 = filtered_df.groupby('Líneas de la orden')['Monto Neto']\
    ...
    grp = filtered_df.groupby('Cliente/Nombre principal')['Monto Neto']\
        .sum().reset_index()
    fig3 = px.pie(
    ...
    with c3:
        buf_pdf3 = generate_pdf(
            report_df, "Resumen de Ventas - ASEAVNA",
            "resumen_ventas_aseavna.pdf"
        )
        st.download_button(
